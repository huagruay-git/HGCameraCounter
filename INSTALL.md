...
## 🔄 วิธีอัปเดตเวอร์ชัน (เครื่องที่ติดตั้งแล้ว)

**ปกติไม่ต้องทำอะไร** — เวอร์ชัน 0.3.3 ขึ้นไปจะ **เช็ค + อัปเดตเอง** ตอนเปิดโปรแกรม/รายวัน (URL ตั้งมาให้ในตัวแล้ว) หรือ HQ สั่ง `update_now` จากระยะไกลได้

**อัปเดตด้วยมือ (เช่น เครื่องที่ยังเป็นเวอร์ชันเก่ามากๆ ที่ยังไม่มีระบบอัปเดตอัตโนมัติ):**
- เปิด **PowerShell as administrator** วางคำสั่งเดียว (ตัวเดียวกับตอนติดตั้ง — มันจะ `git pull` ดึงโค้ดล่าสุดให้):
```powershell
irm https://gist.githubusercontent.com/huagruay-git/7a28954d8b28419ec911659c9838a419/raw/hgcc-install.ps1 | iex
```
- เสร็จแล้ว **ปิด-เปิดโปรแกรมใหม่** → ได้เวอร์ชันล่าสุด (ข้อมูล/config/โมเดล ไม่หาย)
- หรือในโปรแกรม: ปุ่ม **Check Updates** → **Install Update** (0.3.3+ ไม่ต้องกรอก URL แล้ว)
